...
with lib; let
  cfg = config.programs.qemu-cold-export;
  ...
  exportScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      for VM in "''${VMS[@]}"; do
        ...
        chain_len=$(qemu-img info --backing-chain --output=json "$active" | grep -c '"filename"' || true)
        ...
        SNAPNAME="coldbak-$(date +%Y%m%d-%H%M%S)"
...
in {
  options.programs.qemu-cold-export = {
